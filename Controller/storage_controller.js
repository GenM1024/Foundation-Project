...
async function loadInventory() {
    try {
        ...
        displayItems('storage-items', items, 'Storage');
        ...
        document.getElementById('storage-items').innerHTML = '<div class="empty-state">Failed to load items</div>';
    }
}
...
function getActionButtons(item, location) {
    ...
    if (role === 'Admin' || role === 'Manager') {
        buttons += `<button class="btn btn-warning" onclick="editItem(${item.item_id}, '${item.item_name}', '${item.item_category}', ${item.quantity}, '${location}', 'storage')">Edit</button>`;
    }
    
    if (role === 'Admin') {
        buttons += `<button class="btn btn-success" onclick="moveItem(${item.item_id}, '${location}', 'Display', ${item.quantity}, '${item.item_name}')">To Display</button>`;
...
if (checkAuth()) {
    ...
    refreshInterval = setInterval(() => {
        console.log('Auto-refreshing Storage inventory...');
        loadInventory();
