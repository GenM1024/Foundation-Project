...
function checkAuth() {
    ...
    // Add to checkAuth() function in each controller
    ...
    // Disable Returns link for Store Clerks
    if (employee.employee_role === 'Store Clerk') {
        const returnsLink = document.getElementById('returns-link');
        returnsLink.classList.add('disabled');
...
async function loadInventory() {
    try {
        ...
        displayItems('display-items', items, 'Display');
        document.getElementById('display-count').textContent = items.length;
        document.getElementById('error-container').innerHTML = '';
        
    } catch (error) {
        console.error('Error loading inventory:', error);
        showError('Failed to load inventory. ' + error.message);
        document.getElementById('display-items').innerHTML = '<div class="empty-state">Failed to load items</div>';
    }
}
...
function getActionButtons(item, location) {
    ...
    if (role === 'Admin' || role === 'Manager') {
        buttons += `<button class="btn btn-warning" onclick="editItem(${item.item_id}, '${item.item_name}', '${item.item_category}', ${item.quantity}, '${location}', 'front_display')">Edit</button>`;
    }
    
    if (role === 'Admin') {
        buttons += `<button class="btn btn-danger" onclick="moveItem(${item.item_id}, '${location}', 'Returns', ${item.quantity}, '${item.item_name}')">To Returns</button>`;
        buttons += `<button class="btn btn-primary" onclick="moveItem(${item.item_id}, '${location}', 'Storage', ${item.quantity}, '${item.item_name}')">To Storage</button>`;
    } else if (role === 'Manager') {
        buttons += `<button class="btn btn-danger" onclick="moveItem(${item.item_id}, '${location}', 'Returns', ${item.quantity}, '${item.item_name}')">To Returns</button>`;
        buttons += `<button class="btn btn-primary" onclick="moveItem(${item.item_id}, '${location}', 'Storage', ${item.quantity}, '${item.item_name}')">To Storage</button>`;
    } else if (role === 'Store Clerk') {
        buttons += `<button class="btn btn-danger" onclick="moveItem(${item.item_id}, '${location}', 'Returns', ${item.quantity}, '${item.item_name}')">Request Return</button>`;
    }
...
if (checkAuth()) {
    ...
    refreshInterval = setInterval(() => {
        console.log('Auto-refreshing Display inventory...');
        loadInventory();
